const keyboardsFeatureFlagsMap = {
  enableSetterKeyboardHelpers: process.env.GRAMIO_KEYBOARDS_HELPERS_SETTER === "true"
};
...
function pattern(array, pattern2) {
  return customWrap(
    array,
    ({ row, rowIndex }) => row.length === pattern2[rowIndex]
  );
}
function filter(array, fn) {
  ...
  for (const [rowIndex, row] of array.entries()) {
    const filtered = row.filter(
      (button, index) => fn({ button, index, row, rowIndex })
    );
...
class BaseKeyboardConstructor {
  rows = [];
  currentRow = [];
  featureFlags = keyboardsFeatureFlagsMap;
  ...
  appliedFilter;
  get keyboard() {
    let keyboard = this.currentRow.length ? [...this.rows, this.currentRow] : this.rows;
  ...
  columns(length) {
    ...
    else this.appliedHelper = void 0;
    return this;
  }
  /**
   ...
   */
  pattern(pattern2) {
    this.wrapOptions.pattern = pattern2;
    if (pattern2) this.appliedHelper = { type: "pattern", pattern: pattern2 };
    else this.appliedHelper = void 0;
    return this;
  }
  /**
  ...
  add(...buttons) {
    if (this.featureFlags.enableSetterKeyboardHelpers) {
      const applyFilter = this.appliedFilter ? (button) => this.appliedFilter?.({
        button,
      ...
      if (this.appliedHelper?.type === "columns") {
        ...
        for (const button of buttons) {
          if (applyFilter && !applyFilter(button)) continue;
          this.currentRow.push(button);
          if (this.appliedHelper && this.currentRow.length >= this.appliedHelper.pattern[0]) {
            this.row();
            this.appliedHelper.pattern.shift();
            if (this.appliedHelper.pattern.length === 0)
              this.appliedHelper = void 0;
          }
        }
      } else {
  ...
  addIf(condition, ...buttons) {
    const isShow = typeof condition === "boolean" ? condition : condition({
      rowIndex: this.rows.length - 1,
      index: this.currentRow.length - 1
    });
    if (isShow) this.add(...buttons);
    return this;
  }
  /**
  ...
  resetHelpers() {
    this.appliedHelper = void 0;
    this.appliedFilter = void 0;
    return this;
  }
}

class Keyboard extends BaseKeyboardConstructor {
  options = {
    isOneTime: false,
    isPersistent: false,
    isResized: true,
   ...
   * ```
   */
  requestUsers(text, requestId, options = {}, buttonOptions) {
    return this.add(Keyboard.requestUsers(text, requestId, options, buttonOptions));
  }
  /**
   * If specified, pressing the button will open a list of suitable users. Identifiers of selected users will be sent to the bot in a “users\_shared” service message. Available in private chats only.
   */
  static requestUsers(text, requestId, options = {}, buttonOptions) {
    return {
      text,
      request_users: {
        ...options,
   ...
   * ```
   */
  requestChat(text, requestId, options, buttonOptions) {
    return this.add(Keyboard.requestChat(text, requestId, options, buttonOptions));
  }
  /**
   * If specified, pressing the button will open a list of suitable chats. Tapping on a chat will send its identifier to the bot in a “chat\_shared” service message. Available in private chats only.
   */
  static requestChat(text, requestId, options, buttonOptions) {
    return {
      text,
      request_chat: {
   ...
   * ```
   */
  requestManagedBot(text, requestId, options = {}, buttonOptions) {
    return this.add(
   ...
   * If specified, pressing the button will ask the user to create and share a bot that will be managed by the current bot. Available for bots that enabled management of other bots in the [@BotFather](https://t.me/BotFather) Mini App. Available in private chats only.
   */
  static requestManagedBot(text, requestId, options = {}, buttonOptions) {
    return {
      text,
      request_managed_bot: {
        ...options,
   ...
   * ```
   */
  requestContact(text, options) {
    return this.add(Keyboard.requestContact(text, options));
  }
  /**
   * If *True*, the user's phone number will be sent as a contact when the button is pressed. Available in private chats only.
   */
  static requestContact(text, options) {
    return {
      text,
   ...
   * ```
   */
  requestLocation(text, options) {
    return this.add(Keyboard.requestLocation(text, options));
  }
  /**
   * If *True*, the user's current location will be sent when the button is pressed. Available in private chats only.
   */
  static requestLocation(text, options) {
    return {
      text,
   ...
   * If *quiz* is passed, the user will be allowed to create only polls in the quiz mode. If *regular* is passed, only regular polls will be allowed. Otherwise, the user will be allowed to create a poll of any type.
   */
  static requestPoll(text, type, options) {
    return {
      text,
      request_poll: {
        type
   ...
   * ```
   */
  webApp(text, url, options) {
    return this.add(Keyboard.webApp(text, url, options));
  }
  /**
   * If specified, the described [Web App](https://core.telegram.org/bots/webapps) will be launched when the button is pressed. The Web App will be able to send a “web\_app\_data” service message. Available in private chats only.
   */
  static webApp(text, url, options) {
    return {
      text,
      web_app: {
        url
...
class InlineKeyboard extends BaseKeyboardConstructor {
  /**
   ...
   * ```
   */
  text(text, payload, options) {
    return this.add(InlineKeyboard.text(text, payload, options));
  }
  /**
   * Text button with data to be sent in a [callback query](https://core.telegram.org/bots/api/#callbackquery) to the bot when button is pressed, 1-64 bytes
   */
  static text(text, payload, options) {
    return {
      text,
   ...
   * By default empty, in which case just the bot's username will be inserted.
   */
  static switchToChat(text, query = "", options) {
    return {
      text,
   ...
   * ```
   */
  switchToChosenChat(text, query = "", options) {
    return this.add(InlineKeyboard.switchToChosenChat(text, query, options));
  }
  /**
   * Pressing the button will prompt the user to select one of their chats of the specified type, open that chat and insert the bot's username and the specified inline query in the input field
   */
  static switchToChosenChat(text, query = "", options) {
    return {
      text,
   ...
   * This offers a quick way for the user to open your bot in inline mode in the same chat - good for selecting something from multiple options.
   */
  static switchToCurrentChat(text, query = "", options) {
   ...
   * **NOTE:** This type of button **must** always be the first button in the first row.
   */
  static game(text, gameOptions = {}, options) {
    return {
      text,
  ...
  copy(text, textToCopy, options) {
    return this.add(InlineKeyboard.copy(text, textToCopy, options));
  }
  static copy(text, textToCopy, options) {
    return {
      text,
      copy_text: typeof textToCopy === "string" ? { text: textToCopy } : textToCopy,
      ...options
    };
  }
  /**
...
class InputMessageContent {
   ...
   */
  static location(latitude, longitude, options) {
   ...
   */
  static contact(phoneNumber, firstName, options) {
...
class InlineQueryResultCached {
   ...
   */
  static audio(id, fileId, options) {
   ...
   */
  static document(id, title, fileId, options) {
    return {
      type: "document",
      id,
      title,
   ...
   */
  static mpeg4Gif(id, fileId, options) {
   ...
   */
  static photo(id, fileId, options) {
   ...
   */
  static sticker(id, fileId, options) {
   ...
   */
  static video(id, title, fileId, options) {
    return {
      type: "video",
      id,
      title,
   ...
   */
  static voice(id, title, fileId, options) {
    return {
      type: "voice",
      id,
      title,
...
class InlineQueryResult {
   ...
   */
  static game(id, gameShortName, options) {
    return {
      type: "game",
      id,
...
export { BaseKeyboardConstructor, ForceReplyKeyboard, InlineKeyboard, InlineQueryResult, InputMessageContent, Keyboard, RemoveKeyboard, chunk, customWrap, filter, keyboardsFeatureFlagsMap, pattern };
